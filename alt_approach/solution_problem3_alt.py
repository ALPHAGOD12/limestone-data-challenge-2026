"""
Problem 3 (Alt): Buy 100kg of flour only from NaN-price columns, minimizing total cost.

Same BlendV2 strategy as original, but with 7 farmers / 46 indices.
"""
...
print("Problem 3 Alt: Loading imputed data for deviation model training...")
_imp_df = pd.read_csv('alt_approach/imputed_dataset_alt.csv')
_imp_data = _imp_df[_cols].values

_market_level_imp = np.mean(_imp_data, axis=1)
_deviation_imp = _imp_data - _market_level_imp[:, None]

print("Problem 3 Alt: Training deviation correlation models on imputed data...")
_dev_models = {}
...
def trading_problem_3(row):
    ...
    # Reverse inference from observed indices
    solved = {}
    for _ in range(3):
        new_solved = 0
    ...
    return pd.DataFrame({'purchase_col': [_cols[best_col]], 'qty': [100]})


if __name__ == '__main__':
    print("\n" + "="*60)
    print("BACKTEST: trading_problem_3 (ALT) on all historical rows")
    print("="*60)
